...
# Test 3: Content-Length smaller than actual multipart body on server2 -> expect 400
echo "3. POST with Content-Length smaller than body to server2 (expect 400)"
(
  B="BOUNDARY3"
  printf "POST /upload2 HTTP/1.1\r\nHost: server2\r\nConnection: close\r\nContent-Type: multipart/form-data; boundary=$B\r\nContent-Length: 30\r\n\r\n"
  make_multipart "$B" "myfile" "b.txt" "1234567890" "--$B--\r\n" # Actual body > 30 bytes
) | nc localhost 15001 > results/post_fail/fail3.txt &

# Test 4: Content-Length larger than actual multipart body on server3 -> expect 400 or timeout
echo "4. POST with Content-Length larger than body to server3 (expect 400 or timeout)"
(
  B="BOUNDARY4"
  printf "POST /upload3 HTTP/1.1\r\nHost: server3\r\nConnection: close\r\nContent-Type: multipart/form-data; boundary=$B\r\nContent-Length: 200\r\n\r\n"
  make_multipart "$B" "myfile" "c.txt" "short-body" "--$B--\r\n"
) | nc -w 5 localhost 15000 > results/post_fail/fail4.txt &

# Test 5: Invalid Transfer-Encoding chunked (bad chunk size) on server2 (multipart) -> expect 400
...
(
  ...
  printf "0\r\n\r\n"
) | nc localhost 15001 > results/post_fail/fail5.txt &
...
) | nc localhost 15000 > results/post_fail/fail6.txt &
...
curl -i -X POST -H "Host: server1" -H "Connection: close" \
  ...
  http://localhost:15000/upload1 > results/post_fail/fail7.txt &
...
) | nc localhost 15000 > results/post_fail/fail9.txt &
...
curl -i -X POST -H "Host: server1" -H "Connection: close" \
  -F "myfile=@expectedResults/post/upload1/small.txt" \
  http://localhost:15000/ > results/post_fail/fail11.txt &
