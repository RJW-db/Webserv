...
curl -i -H "Connection: close" http://localhost:15000/ > results/post_fail/cookie_request.txt
sleep 0.5
...
curl -i -X POST -H "Host: server1" -H "Connection: close" -H "Cookie: session_id=$session_id" \
...
echo "6. POST malformed multipart (expect 400)"
(
  B="BOUNDARY6"
  printf "POST /upload HTTP/1.1\r\nHost: server3\r\nConnection: close\r\nCookie: session_id=$session_id\r\nContent-Type: multipart/form-data; boundary=$B\r\nContent-Length: 123\r\n\r\n"
...
curl -i -X POST -H "Host: server2" -H "Connection: close" -H "Cookie: session_id=$session_id" \
  -F "myfile=@expectedResults/post/upload1/small.txt" \
