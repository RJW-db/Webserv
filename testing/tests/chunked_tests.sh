...
http://localhost:15000/upload3 > results/chunked/chunked1.txt &

# test 2: Chunked encoding with chunk extensions (should expose validation bug)
echo "2. Testing chunked POST with chunk extensions (exposes chunk validation bug)"
{
    ...
    # Chunk with extension (valid per RFC 7230)
    CHUNK1=$'------WebKitFormBoundary7MA4YWxkTrZu0gW\r\nContent-Disposition: form-data; name="file"; filename="chunk_extension_test.txt"\r\nContent-Type: text/plain\r\n\r\n'
    printf "%X;charset=utf-8\r\n" $(printf "%s" "$CHUNK1" | wc -c)
    printf "%s\r\n" "$CHUNK1"
    
    # Data chunk with extension
    CHUNK2=$'Hello chunk extension test!'
    ...
    # Final chunk
    echo -ne "0\r\n\r\n"
} | nc localhost 15000 > results/chunked/chunked2.txt 2>&1 &
...
echo "3. Testing malformed chunk size (should return 400 Bad Request)"
{
    ...
    echo -ne "GG\r\nInvalid\r\n0\r\n\r\n"
} | nc localhost 15000 > results/chunked/chunked3.txt 2>&1 &
