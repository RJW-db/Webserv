...
void HttpRequest::getContentLength(Client &client)
{
    ...
    if (contentLength == client._headerFields.end())
        throw ErrorCodeClientException(client, BAD_REQUEST, "Content-Length header not found");
    try {
        uint64_t value = stoullSafe(contentLength->second);
        if (static_cast<size_t>(value) > client._location.getClientMaxBodySize())
            throw ErrorCodeClientException(client, PAYLOAD_TOO_LARGE, "Content-Length exceeds maximum allowed: " + to_string(value));
        if (value == 0)
            throw ErrorCodeClientException(client, BAD_REQUEST, "Content-Length cannot be zero.");

        client._contentLength = static_cast<size_t>(value);
    }
    catch (const runtime_error &e) {
        throw ErrorCodeClientException(client, BAD_REQUEST, "Content-Length header is invalid: " + string(e.what()));
    }
}
...
void HttpRequest::getContentType(Client &client)
{
    ...
    if (it == client._headerFields.end())
        throw ErrorCodeClientException(client, BAD_REQUEST, "Content-Type header not found in request");
    ...
    if (ct.find(ContentTypeString[MULTIPART_FORM_DATA]) == 0) { // "multipart/form-data"
        ...
        if (semi != string_view::npos) {
            ...
            if (boundaryPos != string_view::npos)
                ...
                throw ErrorCodeClientException(client, BAD_REQUEST, "Boundary not found in Content-Type header");
        }
        else
            throw ErrorCodeClientException(client, BAD_REQUEST, "Malformed HTTP header line: " + string(ct));
        ...
        if (client._requestPath != "/set-theme" && client._isCgi == false)
            throw ErrorCodeClientException(client, UNSUPPORTED_MEDIA_TYPE, "Unsupported Content-Type for this endpoint: " + string(ct));
    }
    else {
        client._contentType = UNSUPPORTED;
        throw ErrorCodeClientException(client, BAD_REQUEST, "Unsupported Content-Type: " + string(ct));
    }
}
...
void HttpRequest::getBodyInfo(Client &client, const string &buff)
{
    ...
    if (client._filenamePath.empty())
        throw ErrorCodeClientException(client, BAD_REQUEST, "Filename path is empty after processing Content-Disposition");
}
...
void    HttpRequest::appendUuidToFilename(Client &client, string &filename)
{
    ...
    if (totalLength > NAME_MAX) {
        ...
        if (filename.size() > leftOvers)
            ...
            throw ErrorCodeClientException(client, PAYLOAD_TOO_LARGE, "Filename too long to accommodate unique identifier");
    }
    ...
    if (client._filenamePath.size() > PATH_MAX)
        throw ErrorCodeClientException(client, PAYLOAD_TOO_LARGE, "Full path to create file is too long");
}
...
namespace
{
    ...
    void processThemeForm(Client &client)
    {
        ...
        if (client._body.empty() || client._body.size() < 6)
            throw ErrorCodeClientException(client, BAD_REQUEST, "Body too short or empty for form data");

        if (client._body.substr(0, 6) != "theme=")
            throw ErrorCodeClientException(client, BAD_REQUEST, "Unsupported form data key: " + client._body);
        ...
        if (theme == "light")
            ...
            throw ErrorCodeClientException(client, BAD_REQUEST, "Unsupported theme value: " + string(theme));

        string responseStr = HttpRequest::HttpResponse(client, OK, "", 0);
    ...
    string extractContentDispositionLine(Client &client, const string &buff)
    {
        ...
        if (cdPos == string::npos)
            throw ErrorCodeClientException(client, BAD_REQUEST, "Content-Disposition header not found in multipart body");

        size_t wsStart = cdPos + contentDisposition.size();
        size_t wsEnd = buff.find_first_not_of(" \t", wsStart);
        if (wsEnd == string::npos)
            throw ErrorCodeClientException(client, BAD_REQUEST, "Malformed Content-Disposition header: only whitespace found");

        size_t formDataPos = findCaseInsensitive(buff.data() + cdPos, "form-data;");
        if (formDataPos != wsEnd)
            throw ErrorCodeClientException(client, BAD_REQUEST, "Content-Disposition is not form-data");
    ...
    string extractFilenameFromContentDisposition(Client &client, const string &cdLine)
    {
        ...
        if (fnPos == string::npos)
            throw ErrorCodeClientException(client, BAD_REQUEST, "Filename not found in Content-Disposition header");

        size_t fnStart = fnPos + filenameKey.size();
        size_t fnEnd = cdLine.find("\"", fnStart);
        string filename = cdLine.substr(fnStart, fnEnd - fnStart);
        if (filename.empty())
            throw ErrorCodeClientException(client, BAD_REQUEST, "Filename is empty in Content-Disposition header");

        if (filename.find('/') != string::npos || filename.find('\\') != string::npos)
            throw ErrorCodeClientException(client, BAD_REQUEST, "Directory separators not allowed in filename: " + filename);
        
        if (filename == ".." || filename == "." || 
            filename.find("../") != string::npos || filename.find("..\\") != string::npos ||
            filename.find("/..") != string::npos || filename.find("\\..") != string::npos)
            throw ErrorCodeClientException(client, BAD_REQUEST, "Path traversal detected in filename: " + filename);
    ...
    void validateMultipartContentType(Client &client, const string &buff, const string &filename)
    {
        ...
        if (position == string::npos && !filename.empty())
            throw ErrorCodeClientException(client, BAD_REQUEST, "Content-Type header not found in multipart body");
    }
}
