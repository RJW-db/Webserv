...
void HttpRequest::validateHEAD(Client &client)
{
    ...
    if (client._method.empty() || client._version.empty())
        throw ErrorCodeClientException(client, BAD_REQUEST, "Malformed request line");
    ...
    if (client._version != "HTTP/1.1")
        throw ErrorCodeClientException(client, BAD_REQUEST, "Invalid version: " + client._version);
...
namespace
{
    void parseRequestPath(Client &client, string &requestPath)
    {
        ...
        if (isValidAndNormalizeRequestPath(client, requestPath) == false)
            throw ErrorCodeClientException(client, BAD_REQUEST, "Invalid HTTP path: " + requestPath);
    ...
    uint8_t checkAllowedMethod(Client &client)
    {
        ...
        throw ErrorCodeClientException(client, BAD_REQUEST, "Invalid method: " + client._method);
    }
    ...
    void validatePathAndSegmentLengths(Client &client, const vector<string_view> &segments)
    {
        if (client._requestPath.size() > PATH_MAX)
            throw ErrorCodeClientException(client, URI_TOO_LONG, "URI too long");
    ...
    void decodeSafeFilenameChars(Client &client)
    {
        ...
        if (client._rootPath.find('%') != string::npos)
            throw ErrorCodeClientException(client, BAD_REQUEST, "bad filename given by client:" + client._rootPath);
    }
    ...
    void validateResourceAccess(Client &client)
    {
        ...
        if (stat(reqPath.data(), &status) == -1)
            throw ErrorCodeClientException(client, NOT_FOUND, "Couldn't find file: " + reqPath + ", because: " + string(strerror(errno)));

        if (S_ISDIR(status.st_mode) == true)
        {
            if (access(reqPath.data(), R_OK | X_OK) != 0)
                throw ErrorCodeClientException(client, FORBIDDEN, "Forbidden: No permission to access directory");
        ...
        else if (S_ISREG(status.st_mode) == true)
        {
            if (access(reqPath.data(), R_OK) != 0)
                throw ErrorCodeClientException(client, FORBIDDEN, "Forbidden: No permission to read file");
            detectCgiRequest(client);
            if (client._useMethod & (METHOD_HEAD | METHOD_GET))
                client._filenamePath = reqPath;
        }
        else
            throw ErrorCodeClientException(client, NOT_FOUND, "Not a regular file or directory");
    }

    void findIndexFile(Client &client, struct stat &status)
    {
        if (client._requestPath.back() != '/')
            client._requestPath += '/';

        for (const string &indexPage : client._location.getIndexPage())
        {
            // Ensure request path ends with a slash before appending index page
            string fullindexPage = client._location.getRoot() + client._requestPath + indexPage;
            if (stat(fullindexPage.data(), &status) == 0)
            {
                if (S_ISDIR(status.st_mode) == true ||
                    S_ISREG(status.st_mode) == false)
                    continue;
                if (access(fullindexPage.data(), R_OK) == -1)
                {
                    Logger::log(WARN, "Access error", '-', "Access failed: ", strerror(errno));
                    continue;
                }
                client._filenamePath = fullindexPage;
                return;
            }
        }
        ...
        if (client._location.getAutoIndex() == true)
            ...
            throw ErrorCodeClientException(client, NOT_FOUND, "couldn't find index page");
    }
    ...
    void detectCgiRequest(Client &client)
    {
        ...
        if (client._location.isCgiFile(filename) == true)
            ...
            throw ErrorCodeClientException(client, BAD_REQUEST, "request without correct cgi extension");
    }
}
